...
export default defineEventHandler(async (event) => {
  ...
  if (!claims) {
    throw apiError(401, 'AUTH_REQUIRED', 'Sign in to withdraw.')
  }
  ...
  if (registration.status === 'withdrawn') {
    throw apiError(409, 'ALREADY_WITHDRAWN', 'You have already withdrawn.')
  }

  try {
    const updated = await registrationRepo.withdraw(registration.id)
    ...
    console.error('[POST /api/v1/events/:eventId/withdraw] failed:', err)
    throw apiError(500, 'INTERNAL_ERROR', 'Could not withdraw.')
  }
})
