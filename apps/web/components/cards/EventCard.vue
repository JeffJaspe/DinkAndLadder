...
const registrationClass = computed(() => {
  switch (props.registrationStatus) {
    ...
    case 'closed': return 'bg-text-muted/20 text-text-muted'
  }
})
</script>
  ...
  >
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0 flex-1">
        ...
        <div v-if="location" class="mt-1 flex items-center gap-2 text-sm text-text-muted">
          <svg class="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="participantCount !== undefined" class="mt-1 flex items-center gap-2 text-sm text-text-muted">
          <svg class="h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
